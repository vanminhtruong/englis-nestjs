<template>
  <div class="min-h-screen bg-white dark:bg-black p-4 md:p-8">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-8">
        <h1
          class="text-4xl font-bold bg-gradient-to-r from-primary-500 to-secondary-500 bg-clip-text text-transparent mb-4"
        >
          {{ t("favorites.title") }}
        </h1>
        ...
        >
          <template #default="{ hasBackground }">
            ...
            <div
              ...
              class="w-full h-40 mb-4 rounded-xl overflow-hidden"
            >
              <!-- Image Display -->
              <button
                v-if="vocab.image && !vocab.video"
              ...
              <button
                ...
                @click="
                  openVideoPreview({ src: vocab.video, title: vocab.word })
                "
              >
                <img
            ...
            <div class="flex flex-wrap gap-2 mb-4">
              <span
                class="px-2 py-1 bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-lg text-xs font-medium"
                >✓ {{ vocab.correctCount || 0 }}</span
              >
              <span
                class="px-2 py-1 bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg text-xs font-medium"
                >✗ {{ vocab.incorrectCount || 0 }}</span
              >
              <span
                v-for="tag in vocab.tags"
                :key="tag"
                class="px-2 py-1 bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 rounded-lg text-xs font-medium"
                >#{{ tag }}</span
              >
            </div>
...
const { loadFavorites, handleToggleFavorite, handleSpeak, formatDate } =
  useFavoritesHandle(
    setFavorites,
    removeFavorite,
    setLoading,
