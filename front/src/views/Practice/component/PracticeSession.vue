<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center mb-4">
      <div class="flex-1">
        <div class="bg-black/20 dark:bg-white/20 rounded-full h-3 overflow-hidden">
          <div
      ...
      <div class="ml-6 text-center">
        ...
        <div class="text-sm text-black/60 dark:text-white/60">{{ t('practice.timeLeft') }}</div>
      </div>
    </div>

    <!-- Question Card -->
    <div v-if="currentQuestion" class="bg-gradient-to-br from-white to-white/50 dark:from-white/5 dark:to-black border border-black/10 dark:border-white/10 rounded-3xl p-8">
      <div class="text-center mb-8">
        <!-- For image_guess mode, highlight image first -->
        <div v-if="session.mode === 'image_guess'" class="mb-6 flex flex-col items-center gap-4">
          <div
          ...
          >
            <template v-if="currentQuestion.image">
              <img
                :src="currentQuestion.image.startsWith('data:') || currentQuestion.image.startsWith('http') ? currentQuestion.image : `${API_URL}${currentQuestion.image}`"
        ...
        <div v-if="session.mode !== 'image_guess'">
          ...
          <p class="text-xl text-black/50 dark:text-white/50">{{ currentQuestion.pronunciation }}</p>
        </div>
      </div>
      ...
      <div v-if="session.mode === 'flashcard' && !showAnswer" class="space-y-4">
        <div class="text-center py-12">
          ...
          <p class="text-lg text-black/70 dark:text-white/70 mb-6">{{ t('practice.clickToReveal') || 'Click to reveal the meaning' }}</p>
          <button
      ...
      <div v-else-if="session.mode === 'typing' && !showAnswer" class="space-y-4">
        <label class="block text-lg font-medium text-black/70 dark:text-white/70 text-center">
      ...
      <div v-else-if="session.mode === 'image_guess' && !showAnswer" class="space-y-6">
        <label class="block text-lg font-medium text-black/70 dark:text-white/70 text-center">
          {{ t('practice.guessFromImage') || 'Guess the word from the picture' }}
        </label>
        
        <!-- Letter boxes - directly editable -->
        <div class="flex flex-wrap justify-center gap-2" @click="focusHiddenInput">
          <div
            v-for="(letter, index) in getLetterBoxes()"
            :key="index"
            :class="{
              'w-4': letter.originalLetter === ' ',
              'w-12 h-12 border-2 border-black/20 dark:border-white/20 rounded-lg flex items-center justify-center bg-white dark:bg-white/5 text-xl font-bold text-black dark:text-white cursor-text': letter.originalLetter !== ' ',
              'bg-primary-100 dark:bg-primary-900/30 border-primary-300 dark:border-primary-600': letter.isHint,
              'bg-green-100 dark:bg-green-900/30 border-green-300 dark:border-green-600': letter.isUserInput,
              'animate-pulse': letter.originalLetter !== ' ' && !letter.isHint && !letter.isUserInput
            }"
          >
            {{ letter.display }}
          </div>
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="session.mode === 'flashcard'">
          <div class="text-center mb-6">
            <div class="text-3xl font-bold text-primary-500 dark:text-primary-400 mb-4">
              {{ currentQuestion.meaning }}
            </div>
            <div v-if="currentQuestion.example" class="p-4 bg-black/5 dark:bg-white/5 rounded-xl">
              <p class="text-sm italic text-black/60 dark:text-white/60">{{ currentQuestion.example }}</p>
            </div>
          </div>
          
          <div class="text-center mb-4">
            <p class="text-lg text-black/70 dark:text-white/70 mb-4">{{ t('practice.didYouKnow') || 'Did you know this?' }}</p>
        ...
        <div v-else>
          <div class="text-center">
            <div
              :class="{
                'text-green-600 dark:text-green-400': session.answers.get(currentQuestion.vocabularyId)?.isCorrect,
                'text-red-600 dark:text-red-400': !session.answers.get(currentQuestion.vocabularyId)?.isCorrect,
              }"
              class="text-2xl font-bold mb-2"
            >
              {{ session.answers.get(currentQuestion.vocabularyId)?.isCorrect ? '✓ ' + t('practice.correct') : '✗ ' + t('practice.incorrect') }}
            </div>
          </div>
          ...
          <div>
            <div class="space-y-3">
              <div>
                <span class="font-semibold text-black/70 dark:text-white/70">{{ t('practice.yourAnswer') }}:</span>
                <span class="ml-2 text-black dark:text-white">{{ userAnswer }}</span>
              </div>
              <div>
                <span class="font-semibold text-black/70 dark:text-white/70">{{ t('practice.correctAnswer') }}:</span>
                <span class="ml-2 text-black dark:text-white">{{ currentQuestion.meaning }}</span>
              </div>
              <div v-if="currentQuestion.example" class="pt-3 border-t border-black/20 dark:border-white/20">
                <p class="text-sm italic text-black/60 dark:text-white/60">{{ currentQuestion.example }}</p>
              </div>
            </div>
          </div>
    ...
    >
      ...
      >
        <div class="bg-gradient-to-br from-white to-white/50 dark:from-white/5 dark:to-black border border-black/10 dark:border-white/10 rounded-3xl p-8 text-center max-w-lg w-full">
          <div class="text-6xl mb-4">
            🎉
          </div>
          <h2 class="text-3xl font-bold text-black dark:text-white mb-2">{{ t('practice.congratulations') }}</h2>
          <p class="text-black/60 dark:text-white/60 mb-6">{{ t('practice.practiceComplete') }}</p>
          <div class="grid grid-cols-2 gap-4 max-w-md mx-auto mb-6">
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
              <div class="text-3xl font-bold text-blue-600 dark:text-blue-400">{{ totalCorrect }}</div>
              <div class="text-sm text-black/60 dark:text-white/60">{{ t('practice.correctAnswers') }}</div>
            </div>
            <div class="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4">
              <div class="text-3xl font-bold text-purple-600 dark:text-purple-400">{{ totalScore }}</div>
              <div class="text-sm text-black/60 dark:text-white/60">{{ t('practice.score') }}</div>
            </div>
          </div>
          <button
            @click="emit('restart')"
...
import { ref, watch, computed, defineAsyncComponent, onMounted, nextTick } from 'vue'
