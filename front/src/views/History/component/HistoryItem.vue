<template>
  <div class="bg-gradient-to-br from-white to-gray-50 dark:from-white/5 dark:to-white/[0.02] border border-gray-200 dark:border-white/10 rounded-2xl p-6 hover:shadow-lg transition-all">
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mb-2">
              ...
              <span v-if="history.metadata?.isCorrect !== undefined" :class="[
            ...
            <div class="flex flex-wrap gap-2 text-sm">
              <!-- Word Info -->
              <span v-if="history.metadata?.word" class="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 rounded-full">
                📝 {{ history.metadata.word }}
              </span>
              
              <!-- Score (for practice) -->
              <span v-if="history.metadata?.score !== undefined" class="px-3 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 rounded-full">
                🎯 {{ history.metadata.score }} pts
              </span>
              
              <!-- Time (for practice) -->
              <span v-if="history.metadata?.timeSpent" class="px-3 py-1 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded-full">
                ⏱️ {{ formatTime(history.metadata.timeSpent) }}
              </span>
              
              <!-- Date -->
              <span class="px-3 py-1 bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-400 rounded-full">
      ...
      <div class="flex lg:flex-col gap-2 flex-shrink-0">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="inline-block mr-1">
