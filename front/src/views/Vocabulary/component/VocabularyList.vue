    ...
    >
      ...
      <div class="flex-1 w-full min-w-0">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-2">
          <div class="min-w-0">
            <div class="flex items-center gap-2">
              <h3 class="text-2xl font-bold text-gray-900 dark:text-white truncate">
                {{ vocab.word }}
              </h3>
              <button
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 truncate">{{ vocab.pronunciation }}</p>
          </div>
          <span
            :class="{
        ...
        <div class="flex flex-wrap gap-2 mb-3">
          ...
          >
            <component :is="getIconComponent(category.icon)" :color="category.color" class="w-3 h-3" />
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400">
          {{ t('vocabulary.createdAt') }}: {{ formatDate(vocab.createdAt) }}
        </p>
      </div>

      <div class="flex flex-row md:flex-col gap-3 w-full md:w-auto justify-between">
        <div class="flex items-center gap-2">
          <button
            ...
            :title="vocab.isPinned ? t('vocabulary.unpin') : t('vocabulary.pin')"
          >
            <span
              aria-hidden="true"
          ...
          <button
            ...
            :title="vocab.isFavorite ? t('vocabulary.unfavorite') : t('vocabulary.favorite')"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" :fill="vocab.isFavorite ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="2" :class="vocab.isFavorite ? 'text-yellow-500' : 'text-gray-400'">
