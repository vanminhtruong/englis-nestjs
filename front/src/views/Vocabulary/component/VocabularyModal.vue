<template>
  <div v-if="show" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white dark:bg-white/5 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col backdrop-blur-lg">
      ...
      <!-- Content - Scrollable -->
      <form @submit.prevent="onSubmit" class="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div class="relative">
            ...
            >
              <div class="flex items-center gap-2">
                ...
                />
                <span>{{ selectedCategoryData?.name || `-- ${t('vocabulary.selectCategory') || 'Select Category'} --` }}</span>
              </div>
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="transition-transform" :class="{ 'rotate-180': showCategoryDropdown }">
        ...
        <div>
          ...
          <div class="relative">
            ...
            >
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="transition-transform" :class="{ 'rotate-180': showTagDropdown }">
                <polyline points="6 9 12 15 18 9"></polyline>
              </svg>
            </button>
            ...
            >
              ...
              <button
                ...
                :class="{ 'bg-primary-50 dark:bg-primary-900/20': isTagSelected(tag.name) }"
              >
...
function ensureTagsArray() {
  if (!Array.isArray(props.form.tags)) {
    if (typeof props.form.tags === 'string' && props.form.tags.trim().length > 0) {
...
function isTagSelected(tagName: string) {
  return Array.isArray(props.form.tags) && (props.form.tags as string[]).includes(tagName)
}

const isTagLimitReached = computed(() => (props.form.tags as string[]).length >= 3)
