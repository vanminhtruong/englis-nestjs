      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div v-if="vocabulary?.image || vocabulary?.video" class="w-full">
          ...
          <img
            ...
            :src="
              vocabulary.image.startsWith('data:')
                ? vocabulary.image
                : `http://localhost:3000${vocabulary.image}`
            "
            :alt="vocabulary.word"
            class="w-full h-64 object-cover rounded-xl cursor-pointer hover:opacity-90 transition-opacity"
            @click="
              $emit('open-image', {
                src: vocabulary.image.startsWith('data:')
                  ? vocabulary.image
                  : `http://localhost:3000${vocabulary.image}`,
                alt: vocabulary.word,
              })
          ...
          <div
            ...
            class="w-full h-64 rounded-xl overflow-hidden bg-black"
          >
            <iframe
        ...
        <div>
          ...
          <p class="text-lg text-black/60 dark:text-white/60 italic">
        ...
        <div>
          <h4
            class="text-sm font-semibold text-black/70 dark:text-white/70 mb-2"
          >
            Meaning
          </h4>
        ...
        <div v-if="vocabulary?.example">
          ...
          <div class="p-4 bg-black/5 dark:bg-white/5 rounded-xl">
            <p class="text-black/80 dark:text-white/80 italic mb-2">
              {{ vocabulary.example }}
            </p>
            <p
              v-if="vocabulary.exampleTranslation"
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div class="flex gap-2">
              <span
                class="px-3 py-2 bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-lg text-sm font-semibold"
              >
                ✓ {{ vocabulary?.correctCount || 0 }}
              </span>
              <span
                class="px-3 py-2 bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg text-sm font-semibold"
              >
